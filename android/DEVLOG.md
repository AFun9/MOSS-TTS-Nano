...
最后更新：2026-04-18（**M6 v1.0.0 release bundle 接入** — Kotlin 全栈切到官方 4-TTS + 3-Codec 协议；Manifest/ModelConfig/ModelManager/PromptBuilder/InferenceLoop/CodecRunner 整体重写；旧 KvCache/StateSpec/manifest_v2/config.json/state_spec.json 与 inference_traces 全部下线；65 unit tests 全绿，`assembleDebug` ✓；待真机跑 byte-equal trace v2）
...
| 2026-04-17 | **M2.1-M2.4 完成（68 unit tests 全绿）** — `tools/dump_prompt_fixtures.py` 17 个 PromptBuilder 金标准（含 inline audio codes 89.5 KiB）；`PromptBuilder.kt` Kotlin 端 17/17 byte-equal 对齐（途中修了 Tokenizer 全局归一化 → 特殊 token 切片的顺序 bug，否则 1-token off）；`Sampler.kt`（temp/topK/topP/rep-penalty + seeded RNG · 9 tests）；`StateSpec.kt` + `KvCache.kt`（audio_decoder 状态规格解析 + 直接 ByteBuffer 分配 · 12 tests）。 | （待 commit） |
| 2026-04-18 | **M6 v1.0.0 release bundle 接入（Kotlin 协议级重写）** — 上游 `export_v2/release/` 把 5-graph 旧套件换成官方 4 TTS + 3 Codec 共 7 graph 协议（`prefill` / `decode_step` / `local_decoder` / `local_cached_step` × 1 + `encode` / `decode_full` / `decode_step` × 1），`config.json` + `audio_decoder_state_spec.json` 全部内联进 `manifest.json`（`format_version=1`，含 `tts_config` + `prompt_templates` + `generation_defaults` + 每文件 `size`/`sha256`）。`scripts/sync-models-from-export.sh` 改写为 11 文件 hardlink；`android/models/.gitignore` 新增 `*.data`；README 重写。Kotlin 端整体重写：`Manifest.kt`（新 schema）／`ModelConfig.kt`（从 manifest 派生 + 硬编码架构常量 12 layer × 12 head × 64 dim × hidden 768）／`ModelManager.kt`（自校验 SHA-256 + 可选外部 trusted SHA）／`PromptBuilder.kt`（`buildVoiceClonePrompt` 用 manifest 预编码 prefix/suffix token id 数组，杜绝 tokenizer 漂移）／`InferenceLoop.kt` 完全按 `demo_generate.py:generate` 重写：1× prefill → loop { 1× local text head + 16× local audio head（step_type 0/1/2）+ 1× decode_step }，全局 KV 12 层显式滚动 + 每帧 local KV 重置／新增 `CodecRunner.kt`（`encode` + `decodeFull` + 链式 `resampleStereo`/`normalizePeak`，`decodeStep` 流式留 stub）／`OrtTensors.kt` 新增 `intTensor`(i32)、`readFloat3DRow`、`readFloat3DLastRow`。下线：`KvCache.kt`、`StateSpec.kt`、`InferenceLoopTraceTest.kt`、`inference_traces/trace.json`、`manifest_v2_sample.json`、`config_sample.json`、`state_spec_sample.json`、`prompt_fixtures.json`。新单测：`ManifestTest`/`ModelConfigTest`/`ModelManagerTest` 用真实 release manifest 做 fixture，`PromptBuilderTest` 锁布局不变量。**`./gradlew :app:test` 65/65 passed，`:app:assembleDebug` ✓。** 真机 byte-equal trace v2 dump 工具与重写版 `InferenceLoopTraceTest` 留待 M6.1（需要真机回归）。 | （待 commit） |
| 2026-04-17 | **M2.5 框架就绪（待真机验证）** — `android/models/` 用 hardlink 复用 `onnx_export/` 的 INT8 套件（5×onnx + state_spec + config + tokenizer + manifest，158 MB · 零额外占用 · `*.onnx` gitignored），README 写清每个文件角色；`scripts/sync-models-from-export.sh` 应对重导 inode 失效。`tools/dump_inference_trace.py` 用 `do_sample=False` 跑 Python ORT 5 session，3 场景 (zh/en continuation + zh voice_clone) × 30 步，每步 dump 16 codebook frame_tokens + 各层 fingerprint (sha256+sum+abs_sum)，trace.json **416 KiB**（step 0 prefill 用完整 fp、其余用精简，控制体积）。Kotlin 端：`OrtTensors.kt`（int64/f32/bool/empty 张量构造 + 1D/3D 读取 helper）+ `InferenceLoop.kt`（`global_transformer` × 1 + `local_decoder_text` × 1 + `local_decoder_audio` × 16 / 步，按 `_autoregressive_generate` 1:1 复刻，OnnxTensor 显式 close 在 `iterScope` 集中管理）。`pushModels` Gradle typed task（`ExecOperations` · adb push 158 MB → app external files dir · 无 Gradle 9 deprecation）。`InferenceLoopTraceTest`（androidTest · `assumeTrue(modelsDir)` 守门 · 端到端 frame_tokens byte-equal + final codes sha256 + n_generated/eos_step + 早停 first-mismatch 报告）。`compileDebugKotlin` + `compileDebugAndroidTestKotlin` 均 BUILD SUCCESSFUL。**未在真机跑 connectedDebugAndroidTest，L4 byte-equal 是否 pass 尚未验证。** | （待 commit） |
...
| 任务 | 负责人 | 起始日期 | 备注 |
|---|---|---|---|
| **M2.5** InferenceLoop · L4 byte-equal | Cursor + AFun9 | 2026-04-17 | 框架完成；待真机跑 `./gradlew :app:installDebug && :app:pushModels && :app:connectedDebugAndroidTest -Pandroid.testInstrumentationRunnerArguments.class=com.afun.mosstts.core.infer.InferenceLoopTraceTest` 验证 byte-equal。如 pass 即关 M2.5 → M2.6 audio_decoder 出 PCM。 |
